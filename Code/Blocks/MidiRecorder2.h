...
struct MidiRecorder2
{
    ...
    //Inputs
    uint32_t mInputs[6];                                     //Cache input as packed event (note nr, vel, on time, off time)
    ...
    uint8_t mUsedOutputChannels;                            //8 bits to record status of each output, 1 bit for each output.
...
void tickMidiRecorder2(struct MidiRecorder2* data)
{
    //**********************************************
    ...
    *metronomeClockOut = -511;
    ...
    if (0 == midiStart)
    {
        memset(data->mInputs, 0, sizeof(data->mInputs));
        memset(data->mInputRecordEventIds, 0xff, sizeof(data->mInputRecordEventIds));
        memset(data->mInputToOutputMap, 0xff, sizeof(data->mInputToOutputMap));

        memset(data->mOutputPlaybackEventIds,  0xff, sizeof(data->mOutputPlaybackEventIds));
    ...
    for(inputCounter = 0; inputCounter < maxNbInputs; inputCounter++)
    {
        ...
        if(gateIn && (0xff == cachedOutputChannelId))
        {
            //Note-on event.
            noteOnInputChannels[nbNoteOnInputChannels] = inputCounter;
            nbNoteOnInputChannels++;
        }
        else if(!gateIn && (0xff != cachedOutputChannelId))
        {
            //Note-off event.
    ...
    if((eOperationalModeRecord == data->mOperationalMode) && (ePhaseActive == data->mPhase))
    {
        ...
        {
            //Associate the input channel with an event.
        ...
        for(inputCounter = 0; inputCounter < nbNoteOffInputChannels; inputCounter++)
        {
            ...
            const uint16_t deltaNoteOffClock = (noteOffClock > noteOnClock) ? noteOffClock - noteOnClock : 1;
            const uint16_t minDurationNoteOffClock = noteOffClock + deltaNoteOffClock;
            data->mEvents[recordEvent] = MR_INC_NOTE_OFF_CLOCK(recordEvent, minDurationNoteOffClock);

            //Free the input channel.
            data->mInputRecordEventIds[inputChannelId] = 0xff;
        }
    }
    ...
    for(inputCounter = 0; inputCounter < nbNoteOnInputChannels; inputCounter++)
    {
        ...
        uint8_t freeOutputChannelId = 0xff;
        for(outputCounter = 0; outputCounter < maxNbOutputs; outputCounter++)
        {
            const uint8_t isUsed = MR_IS_CHANNEL_USED(data->mUsedOutputChannels, outputCounter);
            if(!isUsed)
            {
                freeOutputChannelId = outputCounter;
                break;
            }
        }

        if(0xff != freeOutputChannelId)
        {
            ...
            data->mInputs[inputChannelId] = MR_PACK_EVENT(0, 0, midiNoteIns[inputChannelId] >> 10, midiVelIns[inputChannelId] >> 3);
        }
        else
        {
            ...
            uint8_t oldestOutputChannelId = 0xff;
            for(outputCounter = 0; outputCounter < maxNbOutputs; outputCounter++)
            {
                const uint8_t candidateEventId = data->mOutputPlaybackEventIds[outputCounter];
                if((0xff != candidateEventId) && (0xfe != candidateEventId))
                {
                    const uint32_t candidateEvent = data->mEvents[candidateEventId];
                    const uint32_t candidateNoteOnClockCount = MR_GET_NOTE_ON_CLOCK(candidateEvent);
        ...
        if(noteOnClockCount == data->mMidiClockCount)
        {
            ...
            else
            {
                ...
                if(oldestOutputChannelId != 0xff)
                {
                    ...
                    data->mEvents[oldestOutputChannelId] = 0xfe;
                }
                else
                {
    ...
    if(ePhaseActive == data->mPhase)
    {
        for(outputCounter = 0; outputCounter < maxNbOutputs;  outputCounter++)
        {
            ...
            if(0xff != playbackEventId && 0xfe != playbackEventId)
            {
                ...
                {
                    data->mOutputPlaybackEventIds[outputCounter] = 0xfe;
                }
            }
        }
    }
    ...
    for(inputCounter = 0; inputCounter < nbNoteOffInputChannels; inputCounter++)
    {
        ...
        data->mInputToOutputMap[inputChannelId] = 0xff;
